...
__NOTE: These are preliminary analyses as of 3/29/18, 2:26 PM__
...
d_post2 <- read_qualtrics_csv("data/Vaccine+PNAS+replication+day3-2018-04-02.csv") %>%
  select(workerId, SC1, contains("Vax3")) %>%
  filter(SC1 == 1) %>%
  rename(
...
gg_marginal_effects(fit_brm_ord2, "phase:condition") %>% plot()
```

### predicting response

Now that we have multiple posttest periods, it might be preferrable to use a model that predicts responses across all phases. This feels a bit less piecemeal than the past models.
...
```{r}
summary(fit_brm_beta2b)
```

```{r}

gg_marginal_effects(fit_brm_beta2b, "phase:condition") %>% plot()
```
...
Part 3 is being conducted today, March 29, 2018. A participante noticed a typo in the "confident" vaccine Intent item, it said "... in the right thing to do" when it should have said "... is the right thing to do".
